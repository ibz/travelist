...
var current_trip;function update_trip_details(trip_id)
{var activity=$("#trip-activity");activity.show();$("#trip-details").load("/trips/"+trip_id+"/details/",null,function(){activity.hide();});}
function edit_points(trip_id)
{var activity=$("#trip-activity");activity.show();$("#trip-details").empty().load("/trips/"+trip_id+"/points/",null,function(){activity.hide();});}
function suggest_place()
{$("#suggest-place").show();$("#add-place,#suggest-place-message").hide();}
function suggest_place_ok()
{var name=$("#suggest-name").val();var comments=$("#suggest-comments").val();if(!name||!comments)
{alert("Please fill in the place name and comments!");return;}
$("#suggest-buttons").hide();$("#suggest-activity").show();$.ajax({type:"POST",url:"/places/suggest/",data:"name="+name+"&comments="+comments,success:function()
{$("#suggest-name,#suggest-comments").val("");$("#suggest-place").hide();$("#add-place,#suggest-place-message").show();$("#suggest-buttons").show();$("#suggest-activity").hide();},error:function()
{$("#suggest-buttons").show();$("#suggest-activity").hide();}});}
function suggest_place_cancel()
{$("#suggest-place").hide();$("#add-place,#suggest-place-message").show();}
...
function add_links()
{$("#existing-links").hide();$("#add-links #users").empty();$("#add-links").show();}
function add_links_invite()
{var user_ids=$.map($("#add-links #users .user-id"),function(el){return $(el).val();}).join(",");if(!user_ids||user_ids=="")
{alert("Add at least one friend before clicking invite.");return;}
var activity=$("#trip-activity");var buttons=$("#add-links #invite,#add-links #cancel");activity.show();buttons.hide();$.post("/trips/"+current_trip.id+"/links/new/",{user_ids:user_ids},function(){$("#add-links").hide();$("#existing-links").show();buttons.show();activity.hide();});}
function add_links_cancel()
{$("#add-links").hide();$("#existing-links").show();}
...
{return;}
$.post("/trips/"+current_trip.id+"/links/"+link_id+"/delete/",{},function()
